...
5. 上传前读取服务器当前版本；首次自动部署允许 `initial-*` 初始版本，之后则要求当前线上 SHA 是本次 Tag SHA 的祖先。否则“防止线上版本倒退”步骤失败，旧 Tag 不会覆盖已经上线的较新版本。
6. 检查通过后，构建产物上传到服务器的临时目录，并解压到 `/var/www/huangjianfen.cn/releases/<40位提交SHA>`。
7. 发布脚本原子切换 `html` 软链接，公网健康检查成功后保留新版本；如果健康检查失败，工作流会有界重试，并且只有 `html` 仍指向本次 SHA 时才自动切回 `previous`。
8. 清理较旧版本，正常情况下保留最近 5 个发布目录，并保护正在使用的版本和上一个版本。
...
- `.github/workflows/deploy.yml`：只响应 `v*` Tag，校验 `main`、测试、构建、防止线上版本倒退、上传、切换、健康检查和清理。
- `scripts/deploy/manage-release.sh`：启用版本、普通手工回退、仅当线上仍是预期 SHA 时的自动回退，以及清理旧版本。
...
if test -e "$DEPLOY_KEY" || test -e "$DEPLOY_KEY.pub"; then
  echo "停止：项目专用密钥路径已经存在。不要覆盖、不要改名继续；请先确认它是否为本项目密钥，或请熟悉 SSH 的人协助处理。"
  return 1 2>/dev/null || exit 1
fi
...
如果看到了“停止”，不要生成新密钥，也不要继续后面的部署配置。本指南只支持并统一使用固定项目专用路径 `$HOME/.ssh/sweet-memories-github-actions`；先确认已有的私钥和 `.pub` 公钥是否确实属于本项目，无法确认时请熟悉 SSH 的人协助处理。绝不要覆盖未知密钥，也不要只在某一段临时改路径。
...
同一时间只运行一个生产部署，正在执行的任务不会因新 Tag 被取消。并发队列最多保留 100 个等待中的运行；超过这个上限时，额外运行会按 GitHub 当前的队列语义被取消，不能假设每一个已推送 Tag 最终都会执行。

排队任务实际开始的顺序不保证与 Tag 推送顺序完全一致。因此工作流在上传前执行“防止线上版本倒退”：读取当前 `html` 指向的线上 SHA，并要求它是本次 Tag SHA 的祖先。如果一个较旧或历史偏离的 Tag 在新版本之后才开始，它会在这一步失败，不会覆盖新版。

日常发布仍不要连续推送多个 Tag。每次只推一个，等待它在 Actions 中结束并完成线上验证后，再创建和推送下一个版本。
...
如果上传、解压或校验在切换前失败，旧版 `html` 不会改变。如果已经切换到新版但公网健康检查失败，工作流会有界重试读取线上链接和执行 `rollback-if-current`：只有确认 `html` 仍指向本次部署 SHA 时才切回 `previous`。如果另一个任务已经改变线上版本，本次失败任务不会无条件覆盖它；工作流会核验最终链接并报告失败。
...
自动回退采用 `rollback-if-current`，只在 `html` 仍是失败任务自己的 SHA 时切回 `previous`，并带有有界重试；它不是无条件的普通 `rollback`。任务结束后用 `readlink -f /var/www/huangjianfen.cn/html` 确认当前目标。修复原因并合并到 `main` 后，用新 Tag 重试，不要复用失败 Tag。
...
4. 只推送这个 Tag，并在 GitHub Actions 和公网页面检查结果；等本次任务结束后再发布下一个 Tag。
...
- 新版健康检查失败时会尝试有条件回退：只有线上仍是本次 SHA 才切回 `previous`。不要在 Actions 仍运行时手动改软链接。
...
本页的原静态前端部署、严格 SSH、Tag 和前端 release 管理流程继续适用。图片 API 的服务器运行时、持久化权限、管理员初始化、五张旧照片迁移、备份恢复、激活门禁和前后端有条件回退，请统一按照 [图片上传 API 运维指南](./photo-upload-operations.md) 执行；不要把其中的高权限命令复制成另一套流程。
