...
本站通过 `public/robots.txt` 发布 `User-agent: *` 和 `Disallow: /`，只请求遵守 robots.txt 的爬虫不要抓取页面。页面中的 `noindex` 等 robots meta 指令，只有在爬虫实际抓取页面并读取到指令后才会生效；如果 URL 已通过其他链接被发现，即使 robots.txt 阻止抓取，它仍可能出现在搜索结果中。这些指令不是密码保护或访问控制，知道地址的任何人仍然可以访问公开页面；要保证隐私，必须使用身份验证或网络访问控制，本项目当前未启用这些限制。
